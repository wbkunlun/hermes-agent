    ...
    def _setup_stream_consumer(self, platform_key):
        ...
        if want_stream_deltas or want_interim_messages:
            try:
                ...
                adapter = self._runner._delivery_adapter_for(ctx.source)
                if adapter:
                    # fork: WeCom routes streaming through the clawrelay-style single-bubble
                    # delivery (think-block UX, 300ms throttle, running indicator — see
                    # plugins/platforms/wecom/stream_delivery.py) instead of
                    # GatewayStreamConsumer. Other platforms take the upstream consumer
                    # construction below.
                    wecom_delivery_cls = getattr(adapter, "WECOM_STREAM_DELIVERY", None)
                    if wecom_delivery_cls is not None:
                        cr_base = os.getenv("WECOM_CHAT_RECORD_BASE_URL", "").strip()
                        stream_consumer = wecom_delivery_cls(
                            adapter=adapter,
                            chat_id=ctx.source.chat_id,
                            chat_record_url=(
                                f"{cr_base.rstrip('/')}/{ctx.session_id}"
                                if cr_base and ctx.session_id
                                else ""
                            ),
                        )
                        # WeComStreamDelivery is the delta sink itself — it always wants deltas
                        # when streaming is on (the fork gate above guarantees this for wecom).
                        consumer_stream_deltas = want_stream_deltas
                    else:
