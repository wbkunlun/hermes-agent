...
class GatewayTurnMixin:
    ...
    async def _hmwa_first_contact_notes(self, source, history, turn_sidecar_notes):
        ...
        # FORK(wbkunlun): sethome prompt disabled by request -- do not nudge users to
        # run /sethome. To restore upstream behavior, remove "True or" from the guard.
        # One-time prompt if no home channel is set (webhooks deliver to configured targets instead).
        if True or not source.platform or source.platform in (Platform.LOCAL, Platform.WEBHOOK):
            return
    ...
    def _proxy_stream_consumer(self, source: "SessionSource", event_message_id, _thread_metadata, _run_still_current, session_id: Optional[str] = None):
        ...
        # fork: WeCom is exempt — it streams by default even with global streaming off, and
        # deciding that needs the explicit per-platform override resolved below.
        if not _scfg.globally_enabled and source.platform != Platform.WECOM:
            return None
        from gateway.display_config import resolve_display_setting
        _plat_streaming = resolve_display_setting(_load_gateway_config(), _platform_config_key(source.platform), "streaming")
        # fork: WeCom streams by default (native stream frames) even when global streaming is
        # off — same rationale as the matching gate in the in-process agent path
        # (gateway/run_turn_runner.py). An explicit display.platforms.wecom.streaming override
        ...
        if source.platform != Platform.WECOM and not _scfg.enabled_for(_plat_streaming):
            return None
        try:
            ...
            if not _adapter:
                return None
            # fork: WeCom routes streaming through the clawrelay-style single-bubble delivery
            # (see plugins/platforms/wecom/stream_delivery.py); other platforms take the
            # upstream consumer construction below.
            _wecom_delivery_cls = getattr(_adapter, "WECOM_STREAM_DELIVERY", None)
            if _wecom_delivery_cls is not None:
                _cr_base = _os.getenv("WECOM_CHAT_RECORD_BASE_URL", "").strip()
                return _wecom_delivery_cls(
                    adapter=_adapter,
    ...
    async def _run_agent_via_proxy(
        ...
        _stream_consumer = (
            ...
            else self._proxy_stream_consumer(source, event_message_id, _thread_metadata, _run_still_current, session_id)
        )
