...
# fork: PreviewedResponseAgent / PreviewedSplitAfterCommentaryAgent were dropped
# upstream, but the fork's final_response_sent discipline tests below still
# exercise them — restored verbatim.
...
async def test_run_agent_queued_message_delivers_first_response_media(monkeypatch, tmp_path):
    ...
    adapter, result = await _run_with_agent(
        ...
        pending_text="queued follow-up",
        # Fork: streaming is enabled by default (StreamingConfig.enabled=True).
        # Pin it off here so this test exercises the direct-send path it was
        # written for (upstream's default); the streamed variant is covered by
        # test_run_agent_queued_message_delivers_streamed_first_response_media.
        config_data={"streaming": {"enabled": False}},
        platform=Platform.DISCORD,
