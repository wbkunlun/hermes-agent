"""slack platform plugin — bundled-plugin entry point.

Thin wrapper that re-exports the canonical adapter from
``gateway.platforms.slack`` (where every fork-specific production
...
``import plugins.platforms.slack`` time.
"""
...
_LAZY_EXPORTS = {
    "SlackAdapter": ("gateway.platforms.slack", "SlackAdapter"),
    "check_slack_requirements": ("gateway.platforms.slack", "check_slack_requirements"),
}
...
def __getattr__(name: str) -> Any:
    ...
    if target is None:
        raise AttributeError(f"module 'plugins.platforms.slack' has no attribute {name!r}")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        key="slack",
        label="Slack",
        platform_enum_member="SLACK",
        adapter_factory=lambda config: SlackAdapter(config),
        requirements_check=check_slack_requirements,
    )
