...
- a ``<think>`` block that stays open while the model is "thinking" and collapses
  ...
  "正在思考" indicator);
- thinking markers ``🤔 正在思考中...`` / ``🔧 **{tool}**`` / ``💭 {preview}`` /
  ``✨ 回复完成``;
...
the input methods (``on_delta`` / ``on_commentary`` / ``on_tool_start`` /
...
class WeComStreamDelivery:
    ...
    def on_segment_break(self) -> None:
        ...
        Tool markers arrive via :meth:`on_tool_start`; we keep accumulating in
