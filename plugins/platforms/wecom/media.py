...
class WeComMediaMixin:
    ...
    async def _cache_media(self, kind: str, media: Dict[str, Any]) -> Optional[Tuple[str, str]]:
        ...
        # fork: inbound video is capped at VIDEO_MAX_BYTES (10MB), same download/decrypt
        # pipeline as files.
        max_bytes = VIDEO_MAX_BYTES if kind == "video" else ABSOLUTE_MAX_BYTES
        try:
            step = "download"
            raw, headers = await self._download_remote_bytes(url, max_bytes=max_bytes)
            step = "decrypt"
        ...
        # fork: WeCom video URLs often carry no usable filename/extension — sniff the
        # container magic so the cached file gets a playable .mp4/.webm suffix.
        ...
        return await self._store_media(kind, raw, ext, content_type, filename, content_type, f" from {url}")
    ...
    def _decode_base64(data: str) -> bytes:
        ...
        # fork: WeCom strips trailing base64 padding in some callback fields
    ...
    def _detect_video_ext(data: bytes) -> str:
        """fork: sniff mp4 (ftyp box at offset 4) / webm (EBML magic); default mp4."""
    ...
    async def _download_remote_bytes(self, url: str, max_bytes: int) -> Tuple[bytes, Dict[str, str]]:
        ...
        # fork: per-deployment SSRF opt-out — in Tencent Cloud VPCs the WeCom media hosts
        # (*.cos.<region>.myqcloud.com) resolve to the 169.254.0.0/16 link-local range for
        # in-VPC free egress, which url_safety blocks unconditionally. These URLs come from
        # the WeCom-signed webhook payload (not LLM-chosen), so operators may disable the
        # check via platforms.wecom.extra.disable_url_safety or WECOM_DISABLE_URL_SAFETY.
        _disable_url_safety = str(
            (self.config.extra or {}).get("disable_url_safety")
        ...
        # fork: create_ssrf_safe_async_client installs a connect-time SSRF guard that the
        # opt-out above cannot turn off (it validates the resolved IP at TCP connect, before
        # any allow-list), so it blocks COS media hosts (169.254.x.x / 198.18.x.x in Tencent
        # VPCs) regardless of the opt-out. When URL safety is disabled, use a plain httpx
        # client while keeping the redirect-only guard as defense in depth — COS downloads
        # do not redirect, so it never fires for them.
        if _disable_url_safety:
            import httpx as _httpx
            client = _httpx.AsyncClient(timeout=30.0, follow_redirects=True, event_hooks={"response": [_ssrf_redirect_guard]})
        else:
