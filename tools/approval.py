...
Fork leaf: ``approval_allowlist`` (operator command allowlist + execute_code parity, gated
before yolo like the deny floor).
...
from tools.approval_allowlist import (
    _execute_code_whitelist_parity, _match_user_allow_rule, _user_allow_block_result,
)
...
def _user_allow_gate(command: str) -> dict | None:
    """Fork: three-state operator allowlist (``HERMES_COMMAND_ALLOWLIST`` / ``approvals.allow``,
    or the control-plane dynamic whitelist when the platform enables it). Like the deny floor
    above this runs BEFORE the yolo / mode=off / cron approve-mode bypass — when an operator
    pins the allowed command set, nothing outside it may run, not even under yolo. Three-state:
    None = feature off (env/config unset, pipeline unchanged); True = explicitly whitelisted,
    short-circuit to approved; False = not whitelisted, hard block. The hardline floor and
    ``approvals.deny`` (checked above) still win, so even an allowlisted ``rm -rf /`` remains
    blocked."""
    _allow = _match_user_allow_rule(command)
    if _allow is False:
        logger.warning("User allowlist blocked command (not whitelisted): %s", command[:200])
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if _should_skip_container_guards(env_type, has_host_access=has_host_access):
        return _user_deny_block(command) or _user_allow_gate(command) or _approved()
    blocked = _floor_block(command)
    if blocked is not None:
        return blocked
    allowed = _user_allow_gate(command)
