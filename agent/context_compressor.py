...
This compaction should PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
    ...
    def compress(
        ...
        # tool-heavy session where the protected tail already holds most of the
        # tokens can burn 500+ seconds on a summary call that replaces a
        ...
        # _session_id keys the per-instance compression circuit breaker (fork).
        # Defined here so the post-call CB recording below always has it, even
        # on the feasibility-skip path (which leaves CB state untouched).
        ...
        if feasibility_skip:
            ...
            # Check compression circuit breaker before LLM call (fork): if the
            # aux model has returned None for N consecutive compressions within
            # a cooldown window, skip the LLM call to avoid blocking the agent
            # thread on every compression cycle.
        ...
        # Record success/failure for circuit breaker. A feasibility skip is a
        # deliberate optimization (middle section too small to be worth an LLM
        # call), not an aux-model failure — leave CB state untouched so skips
